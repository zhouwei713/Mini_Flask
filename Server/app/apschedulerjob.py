...
def opera_db():
    print('cron job start at: ', time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()))

    with scheduler.app.app_context():
        current_app.logger.info('system log cron job start at: %s' % time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()))
        try:
